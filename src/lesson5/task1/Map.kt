...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> =
    stockPrices.groupBy(keySelector = { it.first }, valueTransform = { (it.second) })
        .map { it.key to it.value.average().toString().toDouble() }.toMap()
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    ...
    for ((key, value) in stuff) {
        if (kind == value.first && number > value.second || number == -1.0 && kind == value.first) {
            result = key
            number = value.second
        }
    }
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean =
    word.lowercase().toSet() == word.lowercase().toSet().intersect(chars.map { it.lowercaseChar() }.toSet())
...
fun hasAnagrams(words: List<String>): Boolean =
    words.groupingBy(keySelector = { it.toCharArray().sorted().joinToString() }).eachCount().filterValues { it > 1 }
        .isNotEmpty()
